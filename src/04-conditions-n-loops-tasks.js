...
function findFirstSingleChar(str) {
  ...
  const frequencyMap = chars.reduce((acc, char) => {
    acc[char] = (acc[char] || 0) + 1;
    return acc;
  }, {});

  return chars.find((char) => frequencyMap[char] === 1) || null;
}
...
function getCommonDirectoryPath(pathes) {
  ...
  const commonPath = splitPaths[0].reduce((acc, segment, index) => {
    const isCommon = splitPaths.every((path) => path[index] === segment);
    return isCommon ? [...acc, segment] : acc;
  }, []);
...
function getMatrixProduct(m1, m2) {
  ...
  return result.map((row, i) => row.map((_, j) => m1[i].reduce((sum, _, k) => sum + m1[i][k] * m2[k][j], 0)));
}
